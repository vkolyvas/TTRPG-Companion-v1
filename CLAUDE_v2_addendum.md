...
## ADDENDUM — Synchronized with MVP Strategy v4

> **This addendum extends CLAUDE.md v2. It does not contradict or supersede any
> LOCKED decision in the base document. All locked decisions remain in force.**
> Read the base document first. Apply this addendum on top.
> Changes are surgical: five targeted extensions to existing sections.
> *Addendum version: 2.1*

---

### A1 — DetectionMode Enum Extension

**Affects:** `src-tauri/src/state.rs` and `src/lib/types.ts`

The original `DetectionMode` enum conflates user-chosen interaction style with
system degradation states. These are now distinct concepts and must be separated.

**Replace the existing Rust enum:**

```rust
// REPLACES the DetectionMode enum in §13 (state.rs)
// Old: Full | KeywordOnly | Manual
// New: four values — two user-facing, two system-state
...
pub enum DetectionMode {
    ...
    KeywordOnly,   // System state — not a user choice.
                   ...
                   // Transparent to user — no error state shown. Previously called 'Full'
                   // without speaker verification; renamed for clarity.
...
**Replace the TypeScript type:**

```typescript
// REPLACES DetectionMode in §13 (types.ts)
...
`KeywordOnly` and `Manual` are runtime system states — they are never written
to the config table as a user preference, only surfaced transiently in
`SessionState` and `DetectionModeChangedPayload`.

---

### A2 — Vocal Delivery Threshold: Schema Addition

**Affects:** Encrypted profile DB schema (`profiles/{gm_id}/profile.db`)

The enrollment process now calculates a vocal delivery variance score across the
seven mood passages. This score determines Mode routing (Autonomous vs Collaborative)
and must be persisted so the routing decision survives app restarts without
re-running the calculation.

**Add to the encrypted profile DB migration (new migration file):**

```sql
-- File: src-tauri/src/db/profile_migrations/002_delivery_threshold.sql
-- Adds vocal delivery variance score and mode routing result from enrollment.
-- delivery_variance: float — energy variance across seven mood embeddings,
--   computed as mean pairwise cosine distance between emotion2vec embeddings.
--   Range 0.0–1.0. Higher = more expressive delivery across moods.
-- threshold_passed: 1 if delivery_variance ≥ DELIVERY_THRESHOLD, 0 otherwise.
--   DELIVERY_THRESHOLD is a compile-time constant (see detection/threshold.rs).
-- preferred_mode: persisted user preference. Set from threshold result at
--   enrollment; overridable by user in pre-session setup at any time.
--   Values match DetectionMode::Autonomous and DetectionMode::Collaborative only.
--   System states (KeywordOnly, Manual) are never stored here.
...
**Add constant to detection module:**

```rust
// src-tauri/src/detection/threshold.rs
...
// from diverse GMs during beta. Adjust with documented test evidence only.
pub const DELIVERY_THRESHOLD: f32 = 0.18;
```

---

### A3 — Enrollment Flow: Threshold Step and UX Routing

**Affects:** Module M5 (Voice Enrollment) in §12

The enrollment flow gains a threshold calculation and routing step between
"embeddings extracted" and "profile complete." This step is non-optional —
every enrollment produces a routing decision.

**Extend M5 scope with these steps (append after existing scope):**

```
After all 7 passages are recorded and embeddings extracted:

STEP: Threshold calculation
- Compute mean pairwise cosine distance across the 7 emotion2vec embeddings
- Compare against DELIVERY_THRESHOLD constant
- Write result to enrollment_result table (delivery_variance, threshold_passed,
  threshold_value, enrolled_at)
...
- Do NOT proceed silently to post-enrollment screen regardless of result.
- Two distinct post-enrollment screens, one per routing outcome:
...
### A4 — Config Table: Session Mode Preference and Numpad Mappings

**Affects:** App config table defaults in §13

Two new config keys must be seeded in the config table from day one. Both are
user-overridable in pre-session settings.

**Append to the config table INSERT block:**
...
-- are valid values here. System states are never persisted as preferences.
('session_mode',                   'Autonomous'),

-- Numpad mood mappings — for Mode B GMs who own a numpad or macropad.
-- Default layout mirrors the spatial logic: Calm at center (5),
-- escalation up-right, subdued states lower-left.
-- Values are OS-level key names as understood by Tauri's global hotkey API.
-- Set to empty string to disable numpad for that mood (not null — avoids
-- nullable config complexity).
...
**Implementation note:** Numpad hotkeys are only registered when `session_mode`
is `Collaborative`. They are unregistered on session end or when mode switches
to `Autonomous`. Never register numpad hotkeys in Autonomous mode — they are
Mode B controls and must not fire silently in the background for Autonomous GMs.

---

### A5 — IPC: New Event and Updated Tauri Command Return

**Affects:** §15 IPC & Event Contracts

**Add one new Tauri event:**
...
```typescript
// src/lib/types.ts — add alongside existing payload types
export interface EnrollmentCompletePayload {
  gmId: string;
  deliveryVariance: number;       // 0.0–1.0
  thresholdPassed: boolean;
  assignedMode: 'Autonomous' | 'Collaborative';  // never KeywordOnly or Manual
}
```

This event fires from `complete_enrollment()` after the threshold calculation
and DB writes succeed. The frontend listens for it to decide which post-enrollment
screen to render. Do not derive the routing decision in the frontend by
inspecting `deliveryVariance` against a hardcoded threshold — the backend owns
that logic. The frontend renders based on `assignedMode` from the payload only.

**Update `complete_enrollment` return type:**

```rust
// REPLACES in §15 Tauri Commands
// Old: complete_enrollment(gm_id: String) -> Result<GmProfile, String>
// New: returns EnrollmentSummary which includes routing decision
...
### A6 — Open Questions Register Additions

**Append to §20:**

```
| OQ-11 | DELIVERY_THRESHOLD calibration | 0.18 is an untested hypothesis. Must be
         validated against enrollment data from ≥ 20 diverse GMs during beta —
         expressive performers, monotone narrators, NPC-voice-heavy GMs, quiet
         speakers. Adjust with documented test evidence. Wrong threshold means
         GMs are miscategorised and the mode routing feels broken. | Critical — before G1 |

| OQ-12 | Mode B palette position during session | The mood palette must be visible
         without demanding attention. Proposed: horizontal strip docked to bottom
         of a narrow persistent window (not tray popover — too many clicks).
         Exact dimensions, z-order, and always-on-top behaviour need a design
         spike on both Windows and macOS. | High — M6 dependency |

| OQ-13 | Numpad hotkey conflict detection | Numpad keys conflict with standard
         numeric input when a text field is focused. Tauri's global hotkey API
         fires regardless of focus. Need a rule: numpad hotkeys suppressed when
         any text input has OS focus? Or document as "close text fields during
         sessions"? | Medium — decide before M6 ships |
```

---

*Addendum 2.1 — Synchronized with MVP Strategy v4.*
*Adds: vocal delivery threshold detection and scoring, Mode A (Autonomous) /*
*Mode B (Collaborative) routing, DetectionMode enum extension, enrollment_result*
*schema, post-enrollment UX routing screens, session_mode and numpad config keys,*
*EnrollmentCompletePayload IPC event, EnrollmentSummary command return type.*
